...
class DataLeak {
  ...
  public:
    ...
    void append(uint64_t d) {
      ASSERT(ip, "[Error] IP not set");
      DEBUG(1) printf("DLEAK@%lx: %lx appended\n", ip, d);
      data.push_back(d);
    }
    ...
    void doexport(FILE* f) {
      ...
      ASSERT(!res, "[Error] Unable to write file");
    }
};
...
class CFLeak {
  ...
  public:
    ...
    void append(uint64_t ip) {
      ASSERT(bp, "[Error] BP not set");
      DEBUG(1) printf("CFLEAK@%lx: %lx appended\n", bp, ip);
  ...
  public:  
    ...
    virtual void dleak_create(uint64_t ip) {
      if (dleaks.find(ip) == dleaks.end()) {
        ...
        DEBUG(1) printf("[Warn] DLEAK: %lx not created\n", ip);
      }
    }
    ...
    virtual void cfleak_create(uint64_t ip, uint64_t* mp, uint8_t len) {
      if (cfleaks.find(ip) == cfleaks.end()) {
        ...
        DEBUG(1) printf("[Warn] CFLEAK: %lx not created\n", ip);
      }
    }
    ...
    virtual void dleak_append(uint64_t ip, uint64_t data) {
      if (dleaks.find(ip) == dleaks.end()) {
        DEBUG(1) printf("[Warn] DLEAK: %lx not appended\n", ip);
      } else {
    ...
    virtual void cfleak_append(uint64_t bbl, uint64_t target) {
      if (cfleaks.find(bbl) == cfleaks.end()) {
        DEBUG(1) printf("[Warn] CFLEAK: %lx not appended\n", bbl);
      } else {
  ...
  public:
    ...
    virtual void dleak_append(uint64_t ip, uint64_t data) {
      if (used == false || unknown_child_depth) {
        DEBUG(1) printf("[Warn] DLEAK %lx: skipping due to %d %d\n", ip, used, unknown_child_depth);
      } else {
    ...
    virtual void cfleak_append(uint64_t bbl, uint64_t target) {
      if (used == false || unknown_child_depth) {
        DEBUG(1) printf("[Warn] CFLEAK %lx: skipping due to %d %d\n", bbl, used, unknown_child_depth);
      } else {
...
class AbstractLeakContainer {
  ...
  public:
     ...
     */
    virtual void dleak_create(uint64_t ip) {
      ASSERT(currentContext, "[Error] Context not initialized");
    ...
    virtual void dleak_consume(uint64_t ip, uint64_t data) {
      ASSERT(currentContext, "[Error] Context not initialized");
      DEBUG(1) printf("Consuming DLEAK %lx\n", ip);
    ...
    virtual void cfleak_consume(uint64_t bbl, uint64_t target) {
      ASSERT(currentContext, "[Error] Context not initialized");
      DEBUG(1) printf("Consuming CFLEAK %lx\n", bbl);
...
class CallStack : public AbstractLeakContainer {
  ...
  public:
    ...
    void call_create(uint64_t caller, uint64_t callee) {
      ASSERT(use_callstack, "[Error] Wrong usage of callstack");
      DEBUG(2) printf("Building callstack %lx --> %lx\n", caller, callee);
    ...
    void call_consume(uint64_t caller, uint64_t callee) {
      ASSERT(use_callstack, "[Error] Wrong usage of callstack");
      ASSERT(currentContext, "[Error] Callstack is not initialized");
      DEBUG(3) print_all();
      DEBUG(2) printf("Calling %lx --> %lx\n", caller, callee);
      uint64_t id = get_call_id(caller, callee);
      CallContext* top = static_cast<CallContext*>(currentContext);
      if (!top->used) {
        if (top->caller == caller && top->callee == callee) {
          DEBUG(2) printf("Entered first leaking callstack\n");
          top->used = true;
        }
      } else {
    ...
    void ret_consume(uint64_t ip) {
      ASSERT(use_callstack, "[Error] Wrong usage of callstack");
      ASSERT(currentContext, "[Error] Callstack is not initialized");
      DEBUG(2) printf("Returning %lx\n", ip);
      CallContext* top = static_cast<CallContext*>(currentContext);
      if (top->unknown_child_depth) {
        top->unknown_child_depth--;
      } else {
        if (top->parent) {
          ASSERT(top->parent, "[Error] Callstack parent is empty");
          currentContext = top = top->parent;
        } else {
          DEBUG(2) printf("[Warn] Ignoring return\n");
        }
      }
    }
    ...
    void rewind() {
      ...
      ASSERT(top, "[Error] Leaks not initialized");
      top->used = false;
      currentContext = top;
    }
...
void init() {
  ASSERT(PIN_MutexInit(&lock), "[Error] Mutex init failed");
}
...
{
    ASSERT(threadid == 0, "[Error] Multithreading detected but not supported!");
    PIN_MutexLock(&lock);
    DEBUG(1) printf("Thread begin %d\n",threadid);
    if (thread_state.size() <= threadid) {
      thread_state_t newstate;
      thread_state.push_back(newstate);
    }
    ASSERT(thread_state.size() > threadid, "[Error] thread_state corrupted");
    thread_state[threadid].malloc_pending = false;
    thread_state[threadid].malloc_size = 0;
    thread_state[threadid].realloc_pending = false;
    thread_state[threadid].realloc_old = 0;
    thread_state[threadid].realloc_size = 0;
    thread_state[threadid].calloc_pending = false;
    thread_state[threadid].calloc_size = 0;
...
{
    PIN_MutexLock(&lock);
    DEBUG(1) printf("Thread end %d code %d\n",threadid, code);
    PIN_MutexUnlock(&lock);
}
...
memobj_t* lookup_heap(VOID* addr) {
  ...
  if (heapcache) {
    ASSERT(heapcache->used, "[Error] Heapcache corrupt");
...
VOID test_mem_heap(entry_t* pentry) {
  ...
  if (obj) {
    uintptr_t* paddr = (uintptr_t*)&pentry->data;
    *paddr -= obj->base;
    ASSERT((*paddr & 0xFFFFFFFF00000000ULL) == 0, "[Error] Heap object too big");
    *paddr |= (uintptr_t)obj->id << 32ULL;
...
void record_heap_op(memobj_t *obj, ADDRINT addr) {
  ...
  entry.ip = (void*)(((uintptr_t)obj->id << 32ULL) | obj->size);
...
uint32_t dofree(ADDRINT addr) {
  heapcache = NULL;
  DEBUG(2) std::cout << "Dofree " << std::hex << addr << std::endl;
  if (!addr) {
    return 0;
  }
  for(HEAPVEC::iterator it = heap.begin(); it != heap.end(); ++it) {
 ...
 */
VOID RecordMallocBefore(THREADID threadid, ADDRINT size) {
  if (!Record) return;
  PIN_MutexLock(&lock);
  if (!thread_state[threadid].realloc_pending) {
    DEBUG(1) std::cout << "Malloc called with " << std::hex << size << std::endl;
 ...
 */
VOID RecordMallocAfter(THREADID threadid, ADDRINT addr) {
  if (!Record) return;
  PIN_MutexLock(&lock);
  DEBUG(1) std::cout << "Malloc returned " << std::hex << addr << std::endl;
  ASSERT(thread_state[threadid].malloc_pending == true, "[Error] Malloc returned but not called");
  domalloc(addr, thread_state[threadid].malloc_size, 0);
  thread_state[threadid].malloc_pending = false;
  PIN_MutexUnlock(&lock);
}
 ...
 */
VOID RecordReallocBefore(THREADID threadid, ADDRINT addr, ADDRINT size) {
  if (!Record) return;
  PIN_MutexLock(&lock);
  DEBUG(1) std::cout << "Realloc called with " << std::hex << addr << " " << size << std::endl;
  thread_state[threadid].realloc_old = addr;
 ...
 * @param addr The allocated heap pointer
 */
VOID RecordReallocAfter(THREADID threadid, ADDRINT addr) {
  if (!Record) return;
  PIN_MutexLock(&lock);
  DEBUG(1) std::cout << "Realloc returned " << std::hex << addr << std::endl;
  ...
  domalloc(addr, thread_state[threadid].realloc_size, objid);
  thread_state[threadid].realloc_pending = false;
  PIN_MutexUnlock(&lock);
}
 ...
 */
VOID RecordCallocBefore(THREADID threadid, ADDRINT nelem, ADDRINT size) {
  if (!Record) return;
  PIN_MutexLock(&lock);
 ...
 * @param addr The allocated heap pointer
 */
VOID RecordCallocAfter(THREADID threadid, ADDRINT addr) {
  PIN_MutexLock(&lock);
  ...
  domalloc(addr, thread_state[threadid].calloc_size, 0);
  thread_state[threadid].calloc_pending = false;
  PIN_MutexUnlock(&lock);
}
...
VOID RecordFreeBefore(THREADID threadid, ADDRINT addr) {
  if (!Record) return;
  PIN_MutexLock(&lock);
  DEBUG(1) std::cout << "Free called with " << std::hex << addr << std::endl;
  dofree(addr);
  PIN_MutexUnlock(&lock);
}
...
VOID RecordFunctionEntry_unlocked(THREADID threadid, ADDRINT ins, BOOL indirect, ADDRINT target)
{
  ...
  entry.ip = (void*) ((uintptr_t)ins);
  entry.data = (void*) ((uintptr_t)target);
  DEBUG(3) std::cout << "Call " << std::hex << ins << " to " << target << std::endl;
...
VOID instrumentMainAndAlloc(IMG img, VOID *v)
{
  ...
  if (IMG_Valid(img)) {
    ...
    DEBUG(1) std::cout << "KnobMain: " << KnobMain.Value() << std::endl;
    if (KnobMain.Value().compare("ALL") != 0) {
      ...
      if (mainRtn.is_valid()) {
        ...
        RTN_InsertCall(mainRtn, IPOINT_BEFORE, (AFUNPTR)RecordMainBegin,
          IARG_THREAD_ID,
          IARG_ADDRINT, RTN_Address(mainRtn), IARG_BRANCH_TARGET_ADDR, IARG_END);
      ...
      DEBUG(1) std::cout << "Recording all" << std::endl;
      if (!Record) {
    ...
    if (name.find("libc") != std::string::npos) {
      ...
      if (KnobTrackHeap.Value()) {
        ...
        if (mallocRtn.is_valid()) {
          DEBUG(1) std::cout << "Malloc found in " << IMG_Name(img) << std::endl;
          RTN_Open(mallocRtn);
        ...
        if (reallocRtn.is_valid()) {
          DEBUG(1) std::cout << "Realloc found in " << IMG_Name(img) << std::endl;
          RTN_Open(reallocRtn);
        ...
        if (callocRtn.is_valid()) {
          DEBUG(1) std::cout << "Calloc found in " << IMG_Name(img) << std::endl;
          RTN_Open(callocRtn);
  ...
  if (INS_IsCall(bp)) {
    if (KnobFunc.Value() || KnobBbl.Value()) {
      INS_InsertCall(bp, IPOINT_BEFORE, AFUNPTR(RecordFunctionEntry),
                          ...
                          IARG_BOOL, INS_IsIndirectBranchOrCall(bp),
...
VOID instrumentLeakingInstructions(INS ins, VOID *v)
{
  ...
  if (leaks->get_erase_dleak(ip) || leaks->was_erased_dleak(ip)) {
    ...
    DEBUG(1) printf("Tracing DLEAK %lx\n", ip);
    bool found = instrumentMemIns(ins, true);
    ASSERT(found, "[Error] Memory instruction to instument not found. Have you provided the flag -mem?");
  }
  ...
  if (leaks->get_erase_cfleak(ip) || leaks->was_erased_cfleak(ip)) {
    ...
    DEBUG(1) printf("Tracing CFLEAK %lx\n", ip);
    ...
    while (bp != INS_Invalid()) {
      DEBUG(2) printf("Testing ins %lx\n", INS_Address(bp));
      if (INS_IsBranchOrCall(bp)) {
        DEBUG(2) printf("Found bp %lx\n", INS_Address(bp));
    ...
    ASSERT(found, "[Error] Instruction to instument not found");
  }
}
...
VOID loadLeaks(VOID* v) {
  ...
  ASSERT(f, "[Error] Leak file does not exist");
  fseek(f, 0, SEEK_END);
  long len = ftell(f);
  rewind(f);
  
  DEBUG(1) printf("Reading leaks from %s, size %ld bytes\n", KnobLeakIn.Value().c_str(), len);
  ASSERT(leaks, "[Error] Leaks not initialized");
  while(ftell(f) < len) {
    leakfmt_t elem;
    ASSERT(fread(&elem, sizeof(elem), 1, f) == 1, "[Error] Failed reading leak file");
    uint64_t callee = 0;
    DEBUG(1) printf("Loading leak element %x, %lx, %d\n", elem.type, elem.ip, elem.nopt);
    switch(elem.type) {
      case FUNC_ENTRY:
        ASSERT(elem.nopt == 1, "[Error] Trace format corrupt");
        ASSERT(fread(&callee, sizeof(callee), 1, f) == 1, "[Error] Failed reading leak file");
        if (KnobCallstack.Value()) {
          leaks->call_create(elem.ip, callee);
        }
        DEBUG(1) printf("Func entry %lx\n", callee);
        break;
      case FUNC_EXIT:
        ASSERT(fseek(f, elem.nopt * sizeof(uint64_t), SEEK_CUR) == 0, "[Error] Failed reading leak file");
        if (KnobCallstack.Value()) {
          leaks->ret_create(elem.ip);
        }
        DEBUG(1) printf("Func exit\n");
        break;
      case DLEAK:
        ASSERT(elem.nopt == 0, "[Error] Trace format corrupt");
        leaks->dleak_create(elem.ip);
        DEBUG(1) printf("Adding Dleak: %lx\n", elem.ip);
        break;
      case CFLEAK:
        ASSERT(elem.nopt > 0, "[Error] Trace format corrupt");
        ASSERT(fseek(f, elem.nopt * sizeof(uint64_t), SEEK_CUR) == 0, "[Error] Failed reading leak file");
        leaks->cfleak_create(elem.ip, NULL, 0);
        DEBUG(1) printf("Adding CFleak: %lx\n", elem.ip);
        break;
      default:
        ASSERT(false, "[Error] Invalid leak type");
    }
  }
  ASSERT(ftell(f) == len, "[Error] Trace format corrupt");
...
VOID Fini(INT32 code, VOID *v)
{
  if (!KnobLeaks.Value()) {
    if (!KnobRawFile.Value().empty()) {
      ...
      if (!ftrace) {
        std::cout << "[Error] Unable to open file " << KnobRawFile.Value() << std::endl;
      } else {
        std::cout << "Writing raw results to " << KnobRawFile.Value() << std::endl;
        bool res;
        res = fwrite(&trace[0], sizeof(entry_t), trace.size(), ftrace) == trace.size();
        fclose(ftrace);
        ASSERT(res, "[Error] Unable to write complete trace file. Out of disk memory?");
      }
    }
  /* KnobLeaks is set */
  } else {
    DEBUG(1) leaks->print_all();
    DEBUG(1) printf("Number of uninstrumented data leaks: %lu\n", leaks->get_uninstrumented_dleak_size());
    DEBUG(1) printf("Number of uninstrumented cflow leaks: %lu\n", leaks->get_uninstrumented_cfleak_size());
    DEBUG(1) leaks->print_uninstrumented_leaks();
    
    if (!KnobLeakOut.Value().empty()) {
      ASSERT(!KnobLeakIn.Value().empty(), "[Error] leakout requires leakin");
      ASSERT(leaks, "[Error] Leaks not initialized");
      FILE* fleaks = fopen(KnobLeakOut.Value().c_str(), "w");
      if (!fleaks) {
        std::cout << "[Error] Unable to open file " << KnobLeakOut.Value() << std::endl;
      } else {
        std::cout << "Writing leak results to " << KnobLeakOut.Value() << std::endl;
        if (use_callstack) {
          static_cast<CallStack*>(leaks)->rewind();
        }
...
int main(int argc, char *argv[])
{
  ...
  if (!KnobLeaks.Value()) {
    ...
    DEBUG(1) std::cout << "Tracing leaks" << std::endl;
