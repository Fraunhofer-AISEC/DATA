...
#########################################################################
# @file asymmetric.sh
# @brief Test script for BoringSSL asymmetric ciphers.
...
function cb_prepare_framework {
  ...
  # wget in fetch_boringssl.sh needs system's OpenSSL installation
  # for certificate checks. That's why we must set LD_LIBRARY_PATH
  # *afterwards*
  export OPENSSL="${DATA_COMMON}/openssl/apps/openssl"
  export LD_LIBRARY_PATH="${DATA_COMMON}/openssl"
  export BORINGSSL=${BUILDDIR}/dsawrapper
}
...
#
# $1 ... key file name
# $ALGO ... dsa/dsa_nonce/ec/ec_nonce
# $PARAM ... additional parameters
function cb_genkey {
  TMPLOGFILE=$(mktemp)
  if [[ "${ALGO}" == "ec" || "${ALGO}" == "ec_nonce" ]]; then
    echo "ECDSA with curve ${PARAM}" >> ${TMPLOGFILE}
    if [[ "${PARAM}" == "brainpoolP192r1" ]]; then
        ${BORINGSSL} genpkey "ec" "ec_paramgen_curve:${PARAM}" "ec.param" "$1" &>> ${TMPLOGFILE}
        RES=$((RES + $?))
    else
    ...
    ${BORINGSSL} genpkey "dsa" "dsa_paramgen_q_bits:256" "${ALGO}.param" "$1" &>> ${TMPLOGFILE}
    RES=$((RES + $?))
  else
    pass
  fi
...
function cb_run_command {
  echo "${BORINGSSL}" sign input.bin "${1}" output.bin
}
...
function cb_pre_leakage_model {
  ...
  if [[ "${ALGO}" == "ec_nonce" ]]; then
    if [[ "${CURVE}" == "brainpoolP192r1" ]]; then
        NONCE_EXTRACTOR_BIN="${NONCE_EXTRACTOR_DIR}/bssl_brainpool_nonce"
    else
