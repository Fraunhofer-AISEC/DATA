 ...
 *
 * When instrumenting with IPOINT_AFTER, Intel pin detects function returns by searching for 'ret' instructions
 * in the function body. However, for Ubuntu 16.04 glibc-2.23, calloc seems to do a jmp to another routine
 * rather than a 'ret' instruction. To still detect when calloc returns, we wrap each allocation routine
 * in a separate function that has a 'ret' instruction.
 */
