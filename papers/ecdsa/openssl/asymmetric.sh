...
# If 1, the key is fixed. This is useful for testing nonce leakage.
# Only useful for randomized DSA schemes but not EdDSA
export FIXED_KEY=0
...
function gen_ec_keyfile {
  ALGO=$1
  PARAM=$2
  KEY=$3
  ${OPENSSL} genpkey -paramfile "${ALGO}.param" -out ${KEY}
}
...
function cb_genkey {
  ...
  if [[ "${ALGO}" == "rsa" ]]; then
    ...
    ${OPENSSL} genpkey -genparam -algorithm "ec" -pkeyopt "ec_paramgen_curve:${PARAM}" -out "${ALGO}.param" &>> ${TMPLOGFILE}
    RES=$((RES + $?))
    if [[ "${FIXED_KEY}" -ne "0" ]]; then
      if ! [[ -f constkey.key ]]; then
        gen_ec_keyfile "${ALGO}" "${PARAM}" "constkey.key" &>> ${TMPLOGFILE}
        RES=$((RES + $?))
      fi
      cp constkey.key "${1}"
    else
      gen_ec_keyfile "${ALGO}" "${PARAM}" "${1}" &>> ${TMPLOGFILE}
      RES=$((RES + $?))
    fi
    ...
    if [[ "${FIXED_KEY}" -ne "0" ]]; then
      if ! [[ -f constkey.key ]]; then
        ${OPENSSL} genpkey -paramfile "${ALGO}.param" -out constkey.key &>> ${TMPLOGFILE}
        RES=$((RES + $?))
      fi
      cp constkey.key "${1}"
    else
